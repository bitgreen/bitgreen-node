...
import { BlockNumber, BlockHash, SignedBlock, Extrinsic, EventRecord } from "@polkadot/types/interfaces"
...
export async function processBlock(api: ApiPromise, block_number: BlockNumber | number) {
    ...
    const current_time: string | number | Date = Number(signed_block.block.extrinsics.at(0)?.args.at(0)?.toString())
    ...
    signed_block.block.extrinsics.map(async(ex: Extrinsic, index: number) => {
        ...
        let extrinsic_success = false, new_asset_id: number | undefined
