...
use hex_literal::hex;
use kusama::constants::currency::BITGS as KSM;
use kusama_runtime as kusama;
use pallet_im_online::sr25519::AuthorityId as ImOnlineId;
use pallet_staking::Forcing;
use bitg::constants::currency::BITGS;
use polkadot_primitives::v1::{AccountId, AccountPublic, ValidatorId};
use bitg_runtime as bitg;
use rococo_runtime as rococo;
use rococo_runtime::constants::currency::BITGS as ROC;
use sc_chain_spec::{ChainSpecExtension, ChainType};
use serde::{Deserialize, Serialize};
use sp_core::{crypto::UncheckedInto, sr25519, Pair, Public};
use sp_runtime::{traits::IdentifyAccount, Perbill};
use telemetry::TelemetryEndpoints;
use westend::constants::currency::BITGS as WND;
...
const DEFAULT_PROTOCOL_ID: &str = "bitg";
...
pub type PolkadotChainSpec = service::GenericChainSpec<bitg::GenesisConfig, Extensions>;
...
pub fn bitg_config() -> Result<PolkadotChainSpec, String> {
	PolkadotChainSpec::from_json_bytes(&include_bytes!("../res/bitg.json")[..])
}
...
fn polkadot_staging_testnet_config_genesis(wasm_binary: &[u8]) -> bitg::GenesisConfig {
	...
	const ENDOWMENT: u128 = 1_000_000 * BITGS;
	const STASH: u128 = 100 * BITGS;

	bitg::GenesisConfig {
		pallet_contracts: Some(bitg_runtime::ContractsConfig {
			current_schedule: Default::default(),
		}),
		frame_system: Some(bitg::SystemConfig {
			code: wasm_binary.to_vec(),
			changes_trie_config: Default::default(),
		}),
		
		pallet_balances: Some(bitg::BalancesConfig {
		...
		pallet_indices: Some(bitg::IndicesConfig { indices: vec![] }),
		pallet_session: Some(bitg::SessionConfig {
		...
		pallet_staking: Some(bitg::StakingConfig {
			validator_count: 100,
		...
		pallet_collective_Instance1: Some(bitg::CouncilConfig {
			members: vec![],
			phantom: Default::default(),
		}),
		pallet_collective_Instance2: Some(bitg::TechnicalCommitteeConfig {
			members: vec![],
			phantom: Default::default(),
		}),
		...
		pallet_authority_discovery: Some(bitg::AuthorityDiscoveryConfig { keys: vec![] }),
		claims: Some(bitg::ClaimsConfig {
			claims: vec![],
			vesting: vec![],
		}),

		pallet_vesting: Some(bitg::VestingConfig { vesting: vec![] }),
...
pub fn polkadot_staging_testnet_config() -> Result<PolkadotChainSpec, String> {
	let wasm_binary = bitg::WASM_BINARY.ok_or("bitg development wasm not available")?;
	let boot_nodes = vec![];
	...
	Ok(PolkadotChainSpec::from_genesis(
		...
		Some(
			TelemetryEndpoints::new(vec![(POLKADOT_STAGING_TELEMETRY_URL.to_string(), 0)])
				.expect("bitg Staging telemetry url is valid; qed"),
		),
...
pub fn polkadot_testnet_genesis(
	...
	bitg::GenesisConfig {
		...
		pallet_elections_phragmen: Some(Default::default()),
		pallet_democracy: Some(bitg::DemocracyConfig::default()),
		pallet_collective_Instance1: Some(bitg::CouncilConfig {
			members: vec![],
			phantom: Default::default(),
		}),
...
fn polkadot_development_config_genesis(wasm_binary: &[u8]) -> bitg::GenesisConfig {
...
fn polkadot_local_testnet_genesis(wasm_binary: &[u8]) -> bitg::GenesisConfig {
	polkadot_testnet_genesis(
		wasm_binary,
		vec![
