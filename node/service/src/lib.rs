...
pub use bitg_runtime;
pub use rococo_runtime;
pub use westend_runtime;

native_executor_instance!(
	pub PolkadotExecutor,
	bitg_runtime::api::dispatch,
	bitg_runtime::native_version,
...
>
{
	...
	if config.chain_spec.is_rococo() {
		...
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
			= new_partial::<bitg_runtime::RuntimeApi, PolkadotExecutor>(config)?;
...
pub fn build_light(config: Configuration) -> Result<(TaskManager, RpcHandlers), ServiceError> {
	if config.chain_spec.is_rococo() {
		...
		new_light::<bitg_runtime::RuntimeApi, PolkadotExecutor>(config)
	}
}
...
pub fn build_full(
	...
	if config.chain_spec.is_rococo() {
		...
		new_full::<bitg_runtime::RuntimeApi, PolkadotExecutor>(
			config,
			authority_discovery_disabled,
			grandpa_pause,
