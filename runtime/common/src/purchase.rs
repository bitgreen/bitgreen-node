...
//! Module to process purchase of BITGS.
...
pub trait Trait: frame_system::Trait {
	...
	/// The amount of purchased locked BITGS that we will unlock for basic actions on the chain.
	type UnlockedProportion: Get<Permill>;
	/// The maximum amount of locked BITGS that we will unlock.
...
/// All information about an account regarding the purchase of BITGS.
...
pub struct AccountStatus<Balance> {
	...
	/// The amount of free BITGS they have purchased.
	free_balance: Balance,
	/// The amount of locked BITGS they have purchased.
...
decl_storage! {
	trait Store for Module<T: Trait> as Purchase {
		...
		Statement: Vec<u8>;
		// The block where all locked BITGS will unlock.
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn payout(origin, who: T::AccountId) {
			...
			Accounts::<T>::try_mutate(&who, |status: &mut AccountStatus<BalanceOf<T>>| -> DispatchResult {
				...
				if !status.locked_balance.is_zero() {
					...
					// We allow some configurable portion of the purchased locked BITGS to be unlocked for basic usage.
		...
		/// Set the block where locked BITGS will become unlocked.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[weight = T::DbWeight::get().writes(1)]
