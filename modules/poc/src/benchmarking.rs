...
benchmarks! {
	...
	on_initialize_era {
		...
		for i in 0..1_000 {
			...
			T::Currency::deposit_creating(&voter, BalanceOf::<T>::from(100_001 * BITG));
			T::Currency::deposit_creating(&candidate, BalanceOf::<T>::from(1_000_001 * BITG));

			let _ = Pallet::<T>::start_candidacy(
				RawOrigin::Signed(candidate.clone()).into()
			);

			let amount: BalanceOf<T> = BalanceOf::<T>::from(100_000 * BITG);
	...
	start_candidacy {
		...
		// alice needs funds
		let deposit: BalanceOf<T> = BalanceOf::<T>::from(1_000_001 * BITG);
	...
	commit {
		...
		// alice needs funds
		let deposit: BalanceOf<T> = BalanceOf::<T>::from(100_001 * BITG);
		T::Currency::deposit_creating(&alice, deposit);
	...
	add_funds {
		...
		// alice needs funds
		let deposit: BalanceOf<T> = BalanceOf::<T>::from(200_001 * BITG);
		T::Currency::deposit_creating(&alice, deposit);
