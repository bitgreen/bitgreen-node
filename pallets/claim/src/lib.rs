...
use frame_support::{ensure, weights::Pays, traits::Currency};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn new_deposit(origin: OriginFor<T>, address: Vec<u8>, deposit: Balance) -> DispatchResultWithPostInfo {
			...
			ensure!(BalanceClaim::<T>::contains_key(&address)==false, Error::<T>::DuplicatedAddress);
			// Update deposit
        ...
        #[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn claim_deposit(origin: OriginFor<T>, oldaddress: Vec<u8>,oldpublickey: Vec<u8>,signature: Vec<u8>, recipient:T::AccountId) -> DispatchResultWithPostInfo {
			...
			ensure!(BalanceClaim::<T>::contains_key(&oldaddress)==true, Error::<T>::OldAddressNotfound);
			// check signature from oldchain
			ensure!(Self::verify_signature_ecdsa_address(oldaddress.clone(),signature.clone(),oldpublickey.clone())==true,Error::<T>::WrongSignature);
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn verify_signature_ecdsa_address(oldaddress:Vec<u8>,signature:Vec<u8>,publickey:Vec<u8>) -> bool {
