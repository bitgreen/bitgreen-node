...
use crate::{mock::*, Error, VCUCreationParams, VCUDetail, VCUType, VCUs};
use frame_support::{
...
fn add_new_authorized_accounts_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            VCU::add_authorized_account(RawOrigin::Root.into(), authorised_account_one,),
        ...
        assert_noop!(
            VCU::add_authorized_account(RawOrigin::Root.into(), authorised_account_three,),
...
#[test]
fn remove_authorized_accounts_should_work() {
    new_test_ext().execute_with(|| {
        let authorised_account_one = 1;
        assert_ok!(VCU::add_authorized_account(
            RawOrigin::Root.into(),
        ...
        assert_ok!(VCU::remove_authorized_account(
            RawOrigin::Root.into(),
            authorised_account_one,
        ));
...
#[test]
fn create_vcu_works_for_single() {
    new_test_ext().execute_with(|| {
        let authorised_account_one = 1;
        let project_id = 1;
        let vcu_id = 1;
...
#[test]
fn create_vcu_works_for_bundle() {
    new_test_ext().execute_with(|| {
        let authorised_account_one = 1;
        let project_id = 1;
