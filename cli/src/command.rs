...
impl SubstrateCli for Cli {
	fn impl_name() -> String { "Bitgreen Bitg".into() }
	...
	fn support_url() -> String { "https://github.com/bitgreen/bitg-node/issues/new".into() }

	fn copyright_start_year() -> i32 { 2017 }

	fn executable_name() -> String { "bitg".into() }

	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let id = if id == "" {
			let n = get_exec_name().unwrap_or_default();
			["bitg", "kusama", "westend", "rococo"].iter()
				.cloned()
		...
		Ok(match id {
			...
			"bitg" => Box::new(service::chain_spec::bitg_config()?),
