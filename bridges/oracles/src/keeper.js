/// Keepers listen to requests on both sides of the bridge and allow transfer by reaching a threshold consensus
...
/// handle events from keepers subscription to pallet bridge
const handle_events = async (api, keypair, web3, BitgreenBridge, value) => {
    ...
    switch (value['method']) {
        ...
        case 'MintQueued': {
            ...
            break;
        /// handle Request calls from pallet bridge
        /// take part of Burning consensus if not already in place and is Valid
        case 'Request': {
            ...
            break;
        /// handle Burned event from pallet bridge
        /// after Burning consensus is reached start consensus on the other side of the bridge
        case 'Burned': {
            const signer = value[0];
            // console.log(`signer: \t ${signer}`);
...
/// Subscribe to events on pallet bridge
...
/// handle transfer events in QUEUE_MODE
...
/// handle event from deposit to evm smartcontract
/// Call Mint from pallet bridge if not already in place or threshold reached
