...
contract CompatibilityFallbackHandler is DefaultCallbackHandler, ISignatureValidator {
    ...
    function isValidSignature(bytes calldata _data, bytes calldata _signature) public view override returns (bytes4) {
        ...
        BitgreenBridge safe = BitgreenBridge(payable(msg.sender));
    ...
    function getMessageHash(bytes memory message) public view returns (bytes32) {
        return getMessageHashForSafe(BitgreenBridge(payable(msg.sender)), message);
    }
    ...
    /// @param message Message that should be hashed
    /// @return Message hash.
    function getMessageHashForSafe(BitgreenBridge safe, bytes memory message) public view returns (bytes32) {
